...
SELECT
    str_to_date(concat(yearweek(all_hires.hire_date),' Sunday'),'%X%V %W') AS 'date',
    o.id AS 'ID',
    o.fulfillment AS 'fulfillment',
    tc.utm_medium AS 'Tracking Codes__utm_medium',
    count(distinct all_hires.candidate_id) AS 'weekly_hires_channel_remote'
FROM
    (
        SELECT
...
GROUP BY 
    str_to_date(concat(yearweek(all_hires.hire_date),' Sunday'),'%X%V %W'),
    tc.utm_medium,
    o.fulfillment,
    o.id