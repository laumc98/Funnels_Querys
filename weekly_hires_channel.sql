...
opps_services AS (
    ...
    SELECT
        str_to_date(concat(yearweek(all_hires.hire_date),' Monday'),'%X%V %W') AS 'Date',
        ifnull(os.business_line,'torre_free') AS 'business_line',
        ifnull(os.reach,0) AS reach,
        tc.utm_medium AS 'UTM',
        count(distinct all_hires.candidate_id) AS 'Hire'
    FROM
        (
            SELECT
    ...
    GROUP BY 
        str_to_date(concat(yearweek(all_hires.hire_date),' Monday'),'%X%V %W'),
        os.business_line,
        os.reach,
        tc.utm_medium
) SELECT * FROM weekly_hires;